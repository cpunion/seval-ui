...
export const defaultRegistry: ComponentRegistry = {
    ...
    Image: ({ component, context }) => {
        ...
        return (
            <img src={src} alt={String(alt)} className={`a2ui-image ${isAvatar ? 'avatar' : ''}`} />
        )
    },
    ...
    Button: ({ id, component, context }) => {
        ...
        const action = button.action as { name: string; context?: { key: string; value?: BoundValue }[] } | undefined
        ...
        const content = childId ? context.renderComponent(childId) : label != null ? String(label) : null
